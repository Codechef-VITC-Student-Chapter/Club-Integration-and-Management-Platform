...
function AdminViewCard({ request, onUpdateStatus }) {
  ...
  return (
    rejecttoggle ? <RejectBox onClose={() => { settoggle(false) }} onSubmit={() => { handleDeny() }} title={request.title} points={request.points} username={request.id} toggle={rejecttoggle} /> :
      <div
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mt-6">
          ...
          <div className="flex gap-4 items-center mt-4 md:mt-0 flex-wrap">
            ...
            {request.status !== "pending" && (
              <>
                <button
                  className={`px-6 py-2 rounded text-white transition-all duration-300 ${request.status === "approved"
