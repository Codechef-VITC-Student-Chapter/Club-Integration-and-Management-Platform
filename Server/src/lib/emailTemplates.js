function generateOTPEmailTemplate(otp) {
  ...
  // Generate the styled OTP HTML
  const styledOTP = otpDigits
    .map(
      (digit) =>
        `<span style="display:inline-block;padding:8px 16px;margin:0 4px;background-color:#3D5A80;color:white;border-radius:4px;font-size:24px;font-weight:bold">${digit}</span>`
    )
    .join("");
  ...
  return `
    <div style="width:100%;max-width:600px;margin:0px auto;padding:20px;font-family:Arial,sans-serif">
        ...
        <div style="text-align:center;padding:20px;color:#777777">
            <p style="margin:5px 0">© ${new Date().getFullYear()}CIMP. All rights reserved.</p>
