...
const Department = mongoose.models.Department || mongoose.model('Departments', depsSchema);
...
export const removeDepartment = async (depId) => {
    try {
        const deletedDepartment = await Department.findOneAndDelete({ dep_id: depId });
...
export const removeSubDepartment = async (depId, subDepId) => {
    try {
        ...
        department.subdeps = department.subdeps.filter(subdep => subdep.toString() !== subDepId);
        await department.save();
        return department;
    } catch (error) {
...
export const removeLeadFromDepartment = async (depId, userId) => {
    try {
        ...
        department.leads = department.leads.filter(user => user.toString() !== userId);
        await department.save();
        return department;
    } catch (error) {
